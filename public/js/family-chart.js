...
(function (global, factory) {
    ...
    function otherParent(d, p1, data) {
        return data.find(
            (d0) =>
                d0.id !== p1.id &&
                (d0.id === d.rels.mother || d0.id === d.rels.father)
        );
    }
            ...
            data_stash.forEach((d) => {
                for (let k in d.rels) {
                    ...
                    if (d.rels[k] === datum.id) {
                        ...
                        Array.isArray(d.rels[k]) &&
                        d.rels[k].includes(datum.id)
                    ) {
                        d.rels[k].splice(
    ...
    function handleRelsOfNewDatum({ datum, data_stash, rel_type, rel_datum }) {
        ...
        function addParent(datum) {
            const is_father = datum.data.gender === "M",
                parent_to_add_id =
                    rel_datum.rels[is_father ? "father" : "mother"];
            if (parent_to_add_id)
                removeToAdd(
    ...
    function CalculateTree({
        ...
        function calculateTreePositions(datum, rt, is_ancestry) {
            const hierarchyGetter =
                    rt === "children"
                        ? hierarchyGetterChildren
                        : hierarchyGetterParents,
                d3_tree = d3
                    .tree()
        ...
        function mergeSides(parents, children) {
            ...
            parents.forEach((d) =>
                d.depth === 1 ? (d.parent = children[0]) : null
            );
            ...
            for (let i = tree.length; i--; ) {
                ...
                if (
                    ...
                    d.data.rels.spouses.forEach((sp_id, i) => {
                        ...
                        spouse.sx =
                            i > 0
                                ? spouse.x
                                : spouse.x + (node_separation / 2) * side;
                ...
                if (d.parents && d.parents.length === 2) {
                    const p1 = d.parents[0],
                        ...
                        x = (d, sp) =>
                            midd +
                            (node_separation / 2) * (d.x < sp.x ? 1 : -1);
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if (d.rels.children && d.rels.children.length > 0) {
                    ...
                    d.rels.children.forEach((d0) => {
                        ...
                        if (
                            child.rels[is_father ? "father" : "mother"] !== d.id
                        )
                            return;
                        ...
                        child.rels[!is_father ? "father" : "mother"] =
                            spouse.id;
                    });
                }
            }
    ...
    function positionTree({ t, svg, transition_time = 2000 }) {
        ...
        d3.select(svg)
            ...
            .call(
                zoom.transform,
                d3.zoomIdentity.scale(t.k).translate(t.x, t.y)
            );
    }
        ...
        function handleSpouse({ d }) {
            d.data.rels.spouses.forEach((sp_id) => {
                ...
                links.push({
                    ...
                    _d: () => [
                        d.is_ancestry
                            ? [_or(d, "x") - 0.0001, _or(d, "y")]
                            : [d.x, d.y], // add -.0001 to line to have some length if d.x === spouse.x
                        d.is_ancestry
    ...
    <g class="card-body">
      ...
      <g transform="translate(${card_dim.text_x}, ${card_dim.text_y})">
        <text clip-path="url(#card_text_clip)">
          ...
          <tspan x="${0}" dy="${14}" font-size="10">${card_display[1](
                d.data
            )}</tspan>
        </text>
        <rect width="${card_dim.w - card_dim.text_x - 10}" height="${
                card_dim.h - 20
  ...
  `,
    ...
    <g class="card-body ${card_add ? "card_add" : "card-unknown"}">
      <rect class="card-body-rect" width="${card_dim.w}" height="${
                card_dim.h
            }" fill="rgb(59, 85, 96)" />
  ...
  `,
    ...
    function PencilIcon({ d, card_dim, x, y }) {
        return {
            template: `
    <g transform="translate(${x || card_dim.w - 20},${
                y || card_dim.h - 20
  ...
  `,
    ...
    function LinkBreakIconWrapper({ d, card_dim }) {
        ...
        if (
            (d.is_ancestry || d.data.main) &&
            (areParents(r) || areParents(_r))
        ) {
            g += LinkBreakIcon({
                x: card_dim.w / 2,
                y: 0,
    ...
    function CardImage({ d, image, card_dim, maleIcon, femaleIcon }) {
        return {
            template: `
    <g style="transform: translate(${card_dim.img_x}px,${
                card_dim.img_y
  ...
  `,
      ...
      <g class="genderless-icon">
        <rect height="${
            card_dim.img_h
        }" width="${card_dim.img_w}" fill="rgb(59, 85, 96)" />
        ...
        const data = [
            ...
            },
            {
                ...
                data: {
                    rel_type: "daughter",
                    data: {
                        label: lbls.daughter || "Add daughter",
                        gender: "F",
                    },
                },
            },
    ...
    function View(
        tree,
        { store, data_stash, cont, datum, card_dim, cardEditForm, scale }
    ) {
        ...
        function addEventListeners(view) {
            ...
            function handleCardClick(node) {
                ...
                const card = node.closest(".card"),
                    ...
                    rel_datum = datum,
                    new_datum = createNewPersonWithGenderFromRel({
                        rel_datum,
                        rel_type,
                    }),
                    postSubmit = () => {
    ...
    function Card(props) {
        ...
        return function ({ node, d }) {
            const el = document.createElementNS(
                ...
                gender_class =
                    d.data.data.gender === "M"
                        ? d.data.data.deceased == "1" ? "card-male deceased" : "card-male"
                        : d.data.data.gender === "F"
                        ? d.data.data.deceased == "1" ? "card-female deceased" : "card-female"
                        : "card-genderless",
                card_dim = props.card_dim,
                show_mini_tree = !isAllRelativeDisplayed(
                    d,
                ...
                mini_tree = () =>
                    !d.data.to_add && show_mini_tree
                        ...
                        : "",
                card_body_outline = () =>
                    CardBodyOutline({ d, card_dim, is_new: d.data.to_add })
                        .template,
                card_body = () =>
                    !d.data.to_add
                        ? CardBody({
        ...
        function updateView(props) {
            ...
            function updateLinks() {
                const links_data = tree.data.reduce(
                        (acc, d) =>
                            acc.concat(createLinks({ d, tree: tree.data })),
                        []
                    ),
                    link = view
                        .select(".links_view")
    ...
    function AddRelative({ store, cont, card_dim, cardEditForm, labels }) {
        return function ({ d, scale }) {
            ...
            if (!scale && window.innerWidth < 650)
